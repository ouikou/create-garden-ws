...
const scriptsFileName = cp
    .execSync(`npm pack`, { cwd: path.join(packagesDir, 'garden-scripts') })
    .toString()
    .trim();
const scriptsPath = path.join(packagesDir, 'garden-scripts', scriptsFileName);
const args = process.argv.slice(2);

// Now run the CGW command
const cgwScriptPath = path.join(packagesDir, 'create-garden-ws', 'index.js');
cp.execSync(
